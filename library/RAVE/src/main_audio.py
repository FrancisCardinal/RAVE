...
def run_loop(file_queue, worker_num, DEBUG, MASK, TIMER, SAVE):
    ...
    while not file_queue.empty():
        ...
        # Run audio manager
        audio_manager = AudioManager(debug=DEBUG, mask=MASK, use_timers=TIMER, save_output=SAVE)
        audio_dict = {"name": "loop_sim_source", "type": "sim", "file": audio_file}
        audio_manager.initialise_audio(source=audio_dict)
        audio_manager.main_loop()
        # audio_manager.reset_manager()


def main2(DEBUG, MASK, TIMER, WORKERS, SOURCE_DIR, SAVE):
    ...
    if SOURCE_DIR == "default":
        ...
        if wav_file:
            ...
            audio_dict = {"name": "loop_sim_source", "type": "sim", "file": wav_file[0]}
            audio_manager = AudioManager(debug=DEBUG, mask=MASK, use_timers=TIMER, save_output=SAVE)
...
if __name__ == "__main__":
    ...
    main2(args.debug, args.mask, args.timer, args.workers, source_subfolder, args.save)
