...
def main(TRAIN,
    ...
    if(TRAIN):
        optimizer = torch.optim.SGD(
            eye_tracker_model.parameters(), lr=1e-3, weight_decay=1e-4, momentum=0.9)
        ...
        for images, labels in data_loader:
            ...
            for image, prediction, label in zip(images, predictions, labels):
                image = inverse_normalize(
                    image, EyeTrackerDataset.TRAINING_MEAN, EyeTrackerDataset.TRAINING_STD)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-e', '--nb_epochs', action='store', type=int, default=20,
                        help='Number of epoch for which to train the neural network')
    parser.add_argument('-c', '--continue_training_from_checkpoint',
                        action='store_true', help='Continue training from checkpoint')

    parser.add_argument('-v', '--display_validation', action='store_true',
    ...
    main(args.train, args.nb_epochs, args.continue_training_from_checkpoint,
         args.display_validation, args.predict)
