...
class AudioTrainer(Trainer):
    ...
    def compute_training_loss(self):
        ...
        training_loss = 0.0
        number_of_images = 0
        for images, labels, energy, a, b in tqdm(self.training_loader, "training", leave=False):
    ...
    def show_input_signals(self, signals):
        y2, x2 = np.mgrid[slice(0, 514, 1), slice(0, 2, 2 / math.floor((32000 / 256) + 1))]
    ...
    def compute_validation_loss(self):
        ...
        with torch.no_grad():
            ...
            for images, labels, energy, original_clean_signals, original_audio_freq in tqdm(
                ...
                images, labels, energy, original_clean_signals, original_audio_freq = (
                    images.to(self.device),
                    labels.to(self.device),
                    energy.to(self.device),
                    original_clean_signals.to(self.device),
                    original_audio_freq.to(self.device),
                )
