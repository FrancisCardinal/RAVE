...
def run_generator_loop(source_queue, worker_num, run_params, configs, file_cnt, total_cnt):
    ...
    dataset_builder = AudioDatasetBuilderSim(run_params["OUTPUT"], run_params["DEBUG"], configs)

    dataset_builder.init_sim(
        run_params["SOURCES"], run_params["NOISES"], reverb=run_params["REVERB"], only_speech=run_params["ONLY_SPEECH"]
    )
...
if __name__ == "__main__":
    ...
    # Noise params
    parser.add_argument("-os", "--only_speech", action="store_true", help="Use only speech as noise.")

    # RIR params
    parser.add_argument("-r", "--reverb", action="store_true", help="Use reverberation to generate RIRs")
    ...
    main(source_subfolder, noise_subfolder, output_subfolder, args.debug, args.workers, args.reverb, args.only_speech)
