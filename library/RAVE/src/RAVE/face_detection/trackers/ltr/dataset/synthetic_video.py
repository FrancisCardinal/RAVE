...
class SyntheticVideo(BaseVideoDataset):
    ...
    def __init__(self, base_image_dataset, transform=None):
        ...
        """
        super().__init__(
            base_image_dataset.get_name() + "_syn_vid", base_image_dataset.root, base_image_dataset.image_loader
        )
        ...
        if self.transform is not None:
            if "mask" in anno_frames.keys():
                frame_list, anno_frames["bbox"], anno_frames["mask"] = self.transform(
                    image=frame_list, bbox=anno_frames["bbox"], mask=anno_frames["mask"], joint=False
                )
