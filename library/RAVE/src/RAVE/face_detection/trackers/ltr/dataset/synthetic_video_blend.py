...
class SyntheticVideoBlend(BaseVideoDataset):
    ...
    def __init__(
        self, foreground_image_dataset, background_image_dataset, foreground_transform=None, background_transform=None
    ):
        """
        args:
    ...
    def _paste_target(self, fg_image, fg_box, fg_mask, bg_image, paste_loc):
        ...
        bg_mask = torch.zeros((bg_image.shape[0], bg_image.shape[1], 1), dtype=fg_mask.dtype, device=fg_mask.device)
        ...
        fg_mask_patch = fg_mask[
            fg_box[1] + y1_pad : fg_box[1] + fg_box[3] - y2_pad, fg_box[0] + x1_pad : fg_box[0] + fg_box[2] - x2_pad, :
        ]
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        if self.foreground_transform is not None:
            fg_frame_list, fg_anno_frames["bbox"], fg_anno_frames["mask"] = self.foreground_transform(
                image=fg_frame_list, bbox=fg_anno_frames["bbox"], mask=fg_anno_frames["mask"], joint=False
            )
        ...
        if self.background_transform is not None:
            if "mask" in bg_anno_frames.keys():
                bg_frame_list, bg_anno_frames["bbox"], bg_anno_frames["mask"] = self.background_transform(
                    image=bg_frame_list, bbox=bg_anno_frames["bbox"], mask=bg_anno_frames["mask"], joint=False
                )
            else:
