...
class LWLActor(BaseActor):
    ...
    def __init__(
        self, net, objective, loss_weight=None, num_refinement_iter=3, disable_backbone_bn=False, disable_all_bn=False
    ):
        """
        args:
    ...
    def __call__(self, data):
        ...
        acc_l = [
            davis_jaccard_measure(torch.sigmoid(rm.detach()).cpu().numpy() > 0.5, lb.cpu().numpy())
            for rm, lb in zip(segm_pred.view(-1, *segm_pred.shape[-2:]), gt_segm.view(-1, *segm_pred.shape[-2:]))
        ]
        acc += sum(acc_l)
        cnt += len(acc_l)
        ...
        # Log stats
        stats = {"Loss/total": loss.item(), "Loss/segm": loss_segm.item(), "Stats/acc": acc / cnt}
...
class LWLBoxActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        loss_segm_box = self.loss_weight["segm_box"] * self.objective["segm"](
            mask_pred_box_train, data["train_masks"].view(mask_pred_box_train.shape)
        )
        ...
        acc_lbox = [
            davis_jaccard_measure(torch.sigmoid(rm.detach()).cpu().numpy() > 0.5, lb.cpu().numpy())
            for rm, lb in zip(
                mask_pred_box_train.view(-1, *mask_pred_box_train.shape[-2:]),
                data["train_masks"].view(-1, *mask_pred_box_train.shape[-2:]),
            )
        ]
...
class RTSActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        returns:
            ...
            stats  -  dict containing detailed losses
        """
        segm_pred, target_scores = self.net(
            train_imgs=data["train_images"],
            test_imgs=data["test_images"],
            train_masks=data["train_masks"],
            test_masks=data["test_masks"],
        ...
        last_target_score_reshaped = last_target_score.view(
            -1, last_target_score.shape[-2] * last_target_score.shape[-1]
        )
        ...
        prediction_correct = ((label_val_at_peak >= threshold) & (label_max_vals > 0.25)) | (
            (label_val_at_peak < threshold) & (label_max_vals < 0.25)
        )
