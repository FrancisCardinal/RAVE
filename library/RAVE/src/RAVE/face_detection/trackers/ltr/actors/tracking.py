...
class KLDiMPActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        if "test_clf" in self.loss_weight.keys():
            # Classification losses for the different optimization iterations
            clf_losses_test = [
                self.objective["test_clf"](s, data["test_label"], data["test_anno"]) for s in target_scores
            ]
        ...
        if "clf_ce" in self.loss_weight.keys():
            # Classification losses for the different optimization iterations
            clf_ce_losses = [
                self.objective["clf_ce"](s, data["test_label_density"], grid_dim=(-2, -1)) for s in target_scores
            ]
        ...
        # Log stats
        stats = {"Loss/total": loss.item(), "Loss/bb_ce": bb_ce.item(), "Loss/loss_bb_ce": loss_bb_ce.item()}
        if "test_clf" in self.loss_weight.keys():
...
class KYSActor(BaseActor):
    ...
    def __call__(self, data):
        ...
        for i in range(1, sequence_length):
            ...
            if "fused_score_orig" in aux_data and "test_clf_orig" in self.loss_weight.keys():
                ...
                clf_loss_test_orig_new = self.objective["test_clf"](
                    aux_data["fused_score_orig"], test_label_cur, test_anno_cur, valid_samples=is_valid
                )
        ...
        loss = (
            clf_loss_test_w + dimp_loss_test_w + is_target_loss_w + is_target_after_prop_loss_w + clf_loss_test_orig_w
        )
