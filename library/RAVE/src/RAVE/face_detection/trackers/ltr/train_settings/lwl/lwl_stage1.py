...
def run(settings):
    settings.description = (
        "Default train settings with backbone weights fixed. We initialize the backbone ResNet with "
    ...
    transform_joint = tfm.Transform(
        tfm.ToBGR(), tfm.ToGrayscale(probability=0.05), tfm.RandomHorizontalFlip(probability=0.5)
    )

    transform_train = tfm.Transform(
        tfm.RandomAffine(p_flip=0.0, max_rotation=15.0, max_shear=0.0, max_ar_factor=0.0, max_scale=0.2, pad_amount=0),
    ...
    dataset_train = sampler.LWLSampler(
        [ytvos_train, davis_train],
        [6, 1],
        samples_per_epoch=settings.batch_size * 1000,
        max_gap=100,
        num_test_frames=3,
        num_train_frames=1,
    ...
    dataset_val = sampler.LWLSampler(
        [ytvos_val],
        [1],
        samples_per_epoch=settings.batch_size * 100,
        max_gap=100,
        num_test_frames=3,
        num_train_frames=1,
    ...
    actor = segm_actors.LWLActor(
        net=net, objective=objective, loss_weight=loss_weight, num_refinement_iter=2, disable_all_bn=True
    )

    # Optimizer
    optimizer = optim.Adam(
        [
            {"params": actor.net.target_model.filter_initializer.parameters(), "lr": 5e-5},
