...
def run(settings):
    ...
    base_net_weights = network_loading.load_trained_network(
        settings.env.workspace_dir, "ltr/lwl/lwl_stage2/LWTLNet_ep0080.pth.tar"
    )
    ...
    # Optimizer
    optimizer = optim.Adam([{"params": actor.net.box_label_encoder.parameters(), "lr": 1e-3}], lr=2e-4)
