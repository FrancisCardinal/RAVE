...
def run(settings):
    ...
    settings.print_stats = [
        "Loss/total",
        "Loss/raw/test_clf",
        "Loss/raw/test_clf_acc",
        "Loss/raw/dimp_clf_acc",
        "Loss/raw/is_target",
        "Loss/raw/is_target_after_prop",
    ...
    transform_train = tfm.Transform(
        tfm.ToTensorAndJitter(0.2), tfm.Normalize(mean=settings.normalize_mean, std=settings.normalize_std)
    )
    ...
    dataset_train = sampler.KYSSampler(
        [got10k_train, trackingnet_train, lasot_train],
        [0.3, 0.3, 0.25],
    ...
    dataset_val = sampler.KYSSampler(
        [got10k_val],
        [1],
    ...
    net = kysnet_models.kysnet_res50(
        optim_iter=3,
        cv_kernel_size=3,
    ...
    objective = {
        "test_clf": ltr_losses.LBHingev2(threshold=settings.hinge_threshold, return_per_sequence=False),
    ...
    dimp_jitter_fn = DiMPScoreJittering(
        distractor_ratio=0.1, p_distractor=0.3, max_distractor_enhance_factor=1.3, min_distractor_enhance_factor=0.8
    )
    actor = actors.KYSActor(net=net, objective=objective, loss_weight=loss_weight, dimp_jitter_fn=dimp_jitter_fn)

    optimizer = optim.Adam([{"params": actor.net.predictor.parameters(), "lr": 1e-2}], lr=1e-2)
