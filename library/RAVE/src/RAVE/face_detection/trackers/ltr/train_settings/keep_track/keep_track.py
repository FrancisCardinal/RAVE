...
def run(settings):
    settings.description = (
        "KeepTrack: Trains the target candidate association network using the candidate matching dataset"
    )
    ...
    # Train datasets
    lasot_dumped_train = LasotCandidateMatching(
        settings.env.lasot_dir, settings.env.lasot_candidate_matching_dataset_path, split="train-train"
    )

    # Validation datasets
    lasot_dumped_val = LasotCandidateMatching(
        settings.env.lasot_dir, settings.env.lasot_candidate_matching_dataset_path, split="train-val"
    )

    transform_train = tfm.Transform(
        tfm.ToTensor(), tfm.Normalize(mean=settings.normalize_mean, std=settings.normalize_std)
    )

    img_aug_transform_train = tfm.Transform(
        tfm.ToTensorAndJitter(normalize=True, brightness_jitter=0.5),
        tfm.RandomBlur(sigma=0.5, probability=0.75),
        tfm.Normalize(mean=settings.normalize_mean, std=settings.normalize_std),
    )

    processing_train = processing.TargetCandiateMatchingProcessing(
        output_sz=settings.output_sz,
        train_transform=transform_train,
        num_target_candidates=5,
        img_aug_transform=img_aug_transform_train,
    )

    processing_val = processing.TargetCandiateMatchingProcessing(
        output_sz=settings.output_sz,
        train_transform=transform_train,
        num_target_candidates=5,
        img_aug_transform=img_aug_transform_train,
    ...
    processing_val_real = processing.TargetCandiateMatchingProcessing(
        output_sz=settings.output_sz, train_transform=transform_train, real_target_candidates_only=True
    )

    # Train sampler and loader
    dataset_train = sampler.SequentialTargetCandidateMatchingSampler(
        lasot_dumped_train,
        samples_per_epoch=int(settings.batch_size * 100),
        processing=processing_train,
        sup_modes=["self_sup", "partial_sup"],
        frame_modes=["H", "K", "J"],
        p_frame_modes=[1.0, 0.5, 0.5],
        subseq_modes=["HH", "HK", "HG"],
        p_subseq_modes=[1.0, 0.1, 0.1],
    )

    dataset_val = sampler.SequentialTargetCandidateMatchingSampler(
        lasot_dumped_val,
    ...
    dataset_val_realHH = sampler.SequentialTargetCandidateMatchingSampler(
        lasot_dumped_val,
        samples_per_epoch=int(settings.batch_size * 5),
        processing=processing_val_real,
        sup_modes=["partial_sup"],
        subseq_modes=["HH"],
        p_subseq_modes=[1.0],
    )

    dataset_val_realHK = sampler.SequentialTargetCandidateMatchingSampler(
        lasot_dumped_val,
    ...
    net = tcm.target_candidate_matching_net_resnet50(
        backbone_pretrained=True, frozen_backbone_layers=["conv1", "bn1", "layer1", "layer2"]
    )
