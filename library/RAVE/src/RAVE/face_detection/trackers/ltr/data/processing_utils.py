...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    if output_sz is not None:
        ...
        mask_crop_padded = F.interpolate(
            mask_crop_padded[None, None], (output_sz, output_sz), mode="bilinear", align_corners=False
        )[0, 0]
...
def transform_image_to_crop(
    box_in: torch.Tensor, box_extract: torch.Tensor, resize_factor: float, crop_sz: torch.Tensor
) -> torch.Tensor:
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    if masks is None:
        ...
        crops_resize_factors = [
            sample_target(f, a, search_area_factor, output_sz, m) for f, a, m in zip(frames, box_extract, masks)
        ]
    ...
    box_crop = [
        transform_image_to_crop(a_gt, a_ex, rf, crop_sz) for a_gt, a_ex, rf in zip(box_gt, box_extract, resize_factors)
    ]
...
def sample_target_adaptive(
    im, target_bb, search_area_factor, output_sz, mode: str = "replicate", max_scale_change=None, mask=None
):
    ...
    # Crop image
    crop_sz_x, crop_sz_y = (
        (output_sz * (target_bb[2:].prod() / output_sz.prod()).sqrt() * search_area_factor).ceil().long().tolist()
    )
    ...
    if mask is not None:
        mask_out = F.interpolate(
            mask_crop_padded[None, None], tuple(output_sz.flip(0).long().tolist()), mode="nearest"
        )[0, 0]
...
def target_image_crop(
    ...
    if masks is None:
        frame_crops_boxes = [
            sample_target_adaptive(f, a, search_area_factor, output_sz, mode, max_scale_change)
    ...
    # find the bb location in the crop
    box_crop = [transform_box_to_crop(bb_gt, crop_bb, crop_sz) for bb_gt, crop_bb in zip(box_gt, crop_boxes)]
...
def gaussian_label_function(
    target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False, uni_bias=0
):
