...
class BaseProcessing:
    ...
    def __init__(
        self, transform=transforms.ToTensor(), train_transform=None, test_transform=None, joint_transform=None
    ):
        """
        args:
        ...
        if proposal_method == "default":
            ...
            for i in range(num_proposals):
                proposals[i, :], gt_iou[i] = prutils.perturb_box(
                    box, min_iou=self.proposal_params["min_iou"], sigma_factor=self.proposal_params["sigma_factor"]
                )
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ["train", "test"]:
            ...
            crops, boxes, _ = prutils.jittered_center_crop(
                data[s + "_images"], jittered_anno, data[s + "_anno"], self.search_area_factor, self.output_sz
            )
    ...
    def _get_jittered_box(self, box, mode):
        ...
        if self.scale_jitter_factor.get("mode", "gauss") == "gauss":
            ...
            jittered_size = box[2:4] * torch.exp(
                torch.FloatTensor(2).uniform_(-self.scale_jitter_factor[mode], self.scale_jitter_factor[mode])
            )
        else:
            raise Exception
...
class TargetCandiateMatchingProcessing(BaseProcessing):
    ...
    def _original_and_augmented_frame(self, data: TensorDict):
        ...
        img_coords_pad0, img_coords_pad1 = self._pad_with_fake_candidates(
            img_coords_pad0, img_coords_pad1, valid0, valid1, sa_box0, sa_box1, img.shape
        )
        ...
        gt_assignment[torch.arange(self.num_target_candidates), torch.arange(self.num_target_candidates)] = (
            valid0 * valid1
        )
    ...
    def _pad_with_fake_candidates_drop_gt(self, img_coords, dropout, gt_idx, sa_box, img_shape):
        ...
        for i in range(0, self.num_target_candidates):
            if filled[i] == 0:
                cs = torch.cat(
                    [torch.rand((20, 1)) * (highy - lowy) + lowy, torch.rand((20, 1)) * (highx - lowx) + lowx], dim=1
                )
...
class LTRBDenseRegressionProcessing(BaseProcessing):
    ...
    def _generate_ltbr_regression_targets(self, target_bb):
        ...
        shifts_y = torch.arange(0, self.output_sz, step=self.stride, dtype=torch.float32, device=target_bb.device)
        ...
        xyxy = torch.stack(
            [target_bb[:, 0], target_bb[:, 1], target_bb[:, 0] + target_bb[:, 2], target_bb[:, 1] + target_bb[:, 3]],
            dim=1,
        )
