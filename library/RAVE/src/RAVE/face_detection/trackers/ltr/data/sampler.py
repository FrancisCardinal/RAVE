    ...
    def __getitem__(self, index):
        ...
        while not enough_visible_frames:
            ...
            enough_visible_frames = (
                visible.type(torch.int64).sum().item() > 2 * (self.num_test_frames + self.num_train_frames)
                and len(visible) >= 20
            )
            ...
            if self.frame_sample_mode == "interval":
                ...
                while test_frame_ids is None:
                    base_frame_id = self._sample_visible_ids(
                        visible, num_ids=1, min_id=self.num_train_frames - 1, max_id=len(visible) - self.num_test_frames
                    )
        ...
        if is_video_dataset:
            ...
            while test_frame_ids is None:
                ...
                if not reverse_sequence:
                    ...
                    train_frame_ids = base_frame_id + prev_frame_ids
                    test_frame_ids = self._sample_visible_ids(
                        visible, min_id=0, max_id=train_frame_ids[0] - 1, num_ids=self.num_test_frames
                    )
                ...
                while test_frame_ids is None:
                    ...
                    if occlusion_sampling:
                        ...
                        base_frame_id = self._sample_ids(
                            visible, num_ids=1, min_id=max(0, first_occ_frame - 20), max_id=first_occ_frame - 5
                        )
...
class SequentialTargetCandidateMatchingSampler(torch.utils.data.Dataset):
    def __init__(
        ...
        if p_sup_modes is None:
            self.p_sup_modes = [1.0 / len(self.sup_modes)] * (len(self.sup_modes))
