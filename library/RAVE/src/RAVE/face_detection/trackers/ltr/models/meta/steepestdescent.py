    ...
    def forward(self, meta_parameter: TensorList, num_iter=None, **kwargs):
        ...
        for i in range(num_iter):
            ...
            scores_softmax = activation.softmax_reg(
                scores.reshape(*scores.shape[:2], -1), dim=2, reg=self.softmax_reg
            ).reshape(scores.shape)
            dLds = sample_weight * (scores_softmax - label_density)

            # Compute gradient of loss
            weights_grad = (
                TensorList(torch.autograd.grad(scores, meta_parameter, dLds, create_graph=True))
            ...
            hes_scores_grad = (
                sm_scores_grad
                - scores_softmax * torch.sum(sm_scores_grad, dim=(-2, -1), keepdim=True)
