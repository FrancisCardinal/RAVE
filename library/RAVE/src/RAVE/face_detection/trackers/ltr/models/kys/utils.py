...
class DiMPScoreJittering:
    def __init__(
        ...
        p_distractor=0,
        max_distractor_enhance_factor=1,
    ...
    def __call__(self, score, label):
        ...
        for i in range(num_score_maps):
            score_c[i, dist_id[i]] = self.rand(
                1,
                target_max_val[i] * self.min_distractor_enhance_factor,
        ...
        score_jittered = (
            score * (1.0 - (jitter_score | zero_score).float()).view(num_score_maps, 1).float()
