...
class ResponsePredictor(nn.Module):
    def __init__(
        ...
        self.cost_volume_proc1 = nn.Sequential(
            ...
            conv_block(
                8, 1, kernel_size=cvproc_ksz[1], stride=1, padding=padding_val[1], batch_norm=use_bn, relu=False
            ),
        )
        ...
        for out_dim in representation_predictor_dims:
            representation_predictor_list.append(
                conv_block(in_dim, out_dim, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=True)
            )
            in_dim = out_dim
        ...
        self.response_predictor = nn.Sequential(
            conv_block(in_dim, 1, kernel_size=3, stride=1, padding=1, batch_norm=False, relu=False), nn.Sigmoid()
        )
        ...
        self.is_target_predictor = nn.Sequential(
            conv_block(
                state_dim, 4, kernel_size=gru_ksz, stride=1, padding=int(gru_ksz // 2), batch_norm=False, relu=True
            ),
    ...
    def forward(self, cost_volume, state_prev, dimp_score_cur, init_label=None, dimp_thresh=None, output_window=None):
        ...
        if state_prev is None:
            init_hidden_state = self.init_hidden_state_predictor(
                init_label.view(num_sequences, 1, feat_sz[0], feat_sz[1])
            )
        ...
        state_prev_nhwd = (
            state_prev_ndhw.permute(0, 2, 3, 1)
            .contiguous()
            .view(num_sequences, feat_sz[0] * feat_sz[1], -1, 1, 1)
            .expand(-1, -1, -1, feat_sz[0], feat_sz[1])
        )
        ...
        if self.conf_measure != "none":
            ...
            predictor_input = torch.cat(
                (propagated_h, dimp_score_cur.view(num_sequences, 1, *dimp_score_cur.shape[-2:]), propagation_conf),
                dim=1,
            )
        else:
            predictor_input = torch.cat(
        ...
        scores_cat_pool = (
            F.adaptive_max_pool2d(scores_cat, 1)
            .view(scores_cat.shape[0], scores_cat.shape[1], 1, 1)
