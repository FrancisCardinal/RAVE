...
class LinearFilter(nn.Module):
    ...
    def forward(self, train_feat, test_feat, train_label, *args, **kwargs):
        ...
        # Train filter
        filter, filter_iter, _ = self.get_filter(train_feat, train_label, *args, **kwargs)
    ...
    def get_filter(self, feat, train_label, train_sw, num_objects=None, *args, **kwargs):
        ...
        if self.filter_optimizer is not None:
            weights, weights_iter, losses = self.filter_optimizer(
                TensorList([weights]), feat=feat, label=train_label, sample_weight=train_sw, *args, **kwargs
            )
            weights = weights[0]
