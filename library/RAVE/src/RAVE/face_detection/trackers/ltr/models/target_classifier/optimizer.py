...
class DiMPSteepestDescentGN(nn.Module):
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        for i in range(num_iter):
            ...
            weights_grad = (
                filter_layer.apply_feat_transpose(feat, residuals_mapped, filter_sz, training=self.training)
                + reg_weight * weights
            )
            ...
            alpha_den = (
                (scores_grad * scores_grad).reshape(num_images, num_sequences, -1).sum(dim=(0, 2))
        ...
        if compute_losses:
            ...
            losses.append(
                (((sample_weight * (scores - label_map)) ** 2).sum() + reg_weight * (weights**2).sum())
                / num_sequences
            )
    ...
    def forward(self, weights, feat, bb, sample_weight=None, num_iter=None, compute_losses=True):
        ...
        def _compute_loss(scores, weights):
            return (
                torch.sum(
                    ...
                    * (torch.log(scores.exp().sum(dim=(-2, -1)) + exp_reg) - (label_density * scores).sum(dim=(-2, -1)))
                )
                / num_sequences
        ...
        for i in range(num_iter):
            ...
            scores_softmax = activation.softmax_reg(
                scores.reshape(num_images, num_sequences, -1), dim=2, reg=self.softmax_reg
            ...
            # Compute gradient
            weights_grad = (
                filter_layer.apply_feat_transpose(feat, res, filter_sz, training=self.training) + reg_weight * weights
            )
            ...
            grad_hes_grad = (
                (scores_grad * hes_scores_grad).reshape(num_images, num_sequences, -1).sum(dim=2).clamp(min=0)
            )
