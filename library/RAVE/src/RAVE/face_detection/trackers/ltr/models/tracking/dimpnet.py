...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.classification_layer = (
            [classification_layer] if isinstance(classification_layer, str) else classification_layer
        )
...
def dimpnet18(
    ...
    # Classifier features
    clf_feature_extractor = clf_features.residual_basic_block(
    ...
    initializer = clf_initializer.FilterInitializerLinear(
        filter_size=filter_size, filter_norm=init_filter_norm, feature_dim=out_feature_dim
    )
...
def dimpnet50_simple(
    ...
    residual_module = residual_modules.LinearFilterHinge(
        feat_stride=feat_stride,
        init_filter_reg=optim_init_reg,
...
def L2dimpnet18(
    ...
    # Optimizer for the DiMP classifier
    optimizer = clf_optimizer.DiMPL2SteepestDescentGN(
        num_iter=optim_iter,
        feat_stride=feat_stride,
        init_step_length=optim_init_step,
        hinge_threshold=hinge_threshold,
...
def klcedimpnet18(
    ...
    optim_min_reg=1e-3,
    init_initializer="default",
    normalize_label=False,
    label_shrink=0,
    softmax_reg=None,
    label_threshold=0,
    final_relu=False,
    init_pool_square=False,
    frozen_backbone_layers=(),
):
    ...
    initializer = clf_initializer.FilterInitializerLinear(
        ...
        init_weights=init_initializer,
        pool_square=init_pool_square,
    )

    # Optimizer for the DiMP classifier
    optimizer = clf_optimizer.PrDiMPSteepestDescentNewton(
        num_iter=optim_iter,
        feat_stride=feat_stride,
        init_step_length=optim_init_step,
        init_filter_reg=optim_init_reg,
        ...
        min_filter_reg=optim_min_reg,
        normalize_label=normalize_label,
        label_shrink=label_shrink,
