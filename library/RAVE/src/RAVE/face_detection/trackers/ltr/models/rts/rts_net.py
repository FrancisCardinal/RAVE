...
class RTSNet(nn.Module):
    def __init__(
        ...
        box_label_encoder=None,
        box_label_decoder=None,
        box_target_model=None,
        box_target_model_segm=None,
        bbox_encoder=None,
        segm_encoder=None,
        fusion_module=None,
    ):
    ...
    def forward(
        self, train_imgs, test_imgs, train_masks, test_masks, train_bb, train_label, test_label, num_refinement_iter=2
    ):
        num_sequences = train_imgs.shape[1]
        num_train_frames = train_imgs.shape[0]
        ...
        for i in range(num_test_frames):
            ...
            test_feat_tm_it = test_feat_tm.view(num_test_frames, num_sequences, *test_feat_tm.shape[-3:])[
                i : i + 1, ...
            ]
            ...
            test_feat_backbone_it = {
                k: v.view(num_test_frames, num_sequences, *v.shape[-3:])[i, ...] for k, v in test_feat_backbone.items()
            }
            ...
            if self.clf_enc_input == "baseline":
                ...
                encoded_bbox_label = interpolate(
                    encoded_bbox_labels[i, :, :, :, :], mask_encoding_pred_last_iter.shape[-2:]
                )
            ...
            if (i < (num_test_frames - 1)) and (num_refinement_iter > 0):
                ...
                filter_updated, _, _ = self.target_model.filter_optimizer(
                    TensorList([filter]),
    ...
    def segment_target(self, target_filter, test_feat_tm, test_feat, encoded_clf_scores=None):
        ...
        mask_pred, decoder_feat = self.decoder(
            decoder_input, test_feat, (test_feat_tm.shape[-2] * 16, test_feat_tm.shape[-1] * 16)
        )
...
def build_base_components(
    ...
    label_encoder = seg_label_encoder.ResidualDS16SW(
        layer_dims=label_encoder_dims + (num_filters,), use_bn=use_bn_in_label_enc
    )

    # Predicts initial target model parameters
    initializer = seg_initializer.FilterInitializerZero(
        filter_size=filter_size, num_filters=num_filters, feature_dim=out_feature_dim, filter_groups=filter_groups
    )

    # Computes few-shot learning loss
    residual_module = loss_residual_modules.RTSResidual(
    ...
    clf_residual_module = clf_residual_modules.LinearFilterHinge(
        feat_stride=clf_feat_stride,
        init_filter_reg=optim_init_reg,
        hinge_threshold=clf_hinge_threshold,
        activation_leak=clf_activation_leak,
        score_act=clf_score_act,
        act_param=clf_act_param,
        learn_filter_reg=False,
    )

    # Construct generic optimizer module
    clf_optimizer = steepestdescent.GNSteepestDescent(
        residual_module=clf_residual_module,
        num_iter=optim_iter,
        detach_length=detach_length,
        residual_batch_dim=1,
        compute_losses=True,
    ...
    if clf_with_extractor:
        clf_initializer = clf_init.FilterInitializerLinear(
            filter_size=clf_filter_size, filter_norm=False, feature_dim=out_feature_dim
        )
        ...
        clf_initializer = clf_init.FilterInitializerZero(filter_size=clf_filter_size, feature_dim=out_feature_dim)
        clf_feature_extractor = None

    classifier = clf_target_clf.LinearFilter(
        filter_size=clf_filter_size,
        filter_initializer=clf_initializer,
        filter_optimizer=clf_optimizer,
...
def steepest_descent_resnet50(
    ...
    (
        ...
        optim_init_reg=optim_init_reg,
        backbone_pretrained=backbone_pretrained,
        clf_feat_blocks=clf_feat_blocks,
        clf_feat_norm=clf_feat_norm,
        final_conv=final_conv,
        ...
        detach_length=detach_length,
        label_encoder_dims=label_encoder_dims,
        frozen_backbone_layers=frozen_backbone_layers,
        decoder_mdim=decoder_mdim,
        filter_groups=filter_groups,
        use_bn_in_label_enc=use_bn_in_label_enc,
...
def steepest_descent_resnet50_with_clf_encoder(
    ...
    net = RTSNet(
        ...
        label_encoder=label_encoder,
        classifier=classifier,
        clf_encoder=clf_encoder,
        target_model_input_layer=target_model_input_layer,
        decoder_input_layers=decoder_input_layers,
        clf_enc_input=clf_enc_input,
...
def steepest_descent_resnet50_with_clf_encoder_boxinit(
    ...
    if box_label_encoder_type == "ResidualDS16FeatSWBox":
        ...
        segm_encoder = seg_label_encoder.ResidualDS16SW(
            layer_dims=box_label_encoder_dims[:-1] + (num_filters,), use_bn=use_bn_in_label_enc
        )

        # Computes few-shot learning loss
        box_residual_module = loss_residual_modules.RTSResidual(init_filter_reg=optim_init_reg)
        box_residual_module_segm = loss_residual_modules.RTSResidual(init_filter_reg=optim_init_reg)
        ...
        box_optimizer = steepestdescent.GNSteepestDescent(
            residual_module=box_residual_module,
            num_iter=optim_iter,
            detach_length=detach_length,
            residual_batch_dim=1,
            compute_losses=True,
        ...
        box_optimizer_segm = steepestdescent.GNSteepestDescent(
            residual_module=box_residual_module_segm,
            num_iter=optim_iter,
            detach_length=detach_length,
            residual_batch_dim=1,
            compute_losses=True,
        ...
        box_target_model = target_clf.LinearFilter(
            filter_size=filter_size,
            filter_initializer=box_initializer,
            filter_optimizer=box_optimizer,
            feature_extractor=box_target_model_feature_extractor,
            filter_dilation_factors=dilation_factors,
        )
        box_target_model_segm = target_clf.LinearFilter(
            filter_size=filter_size,
            filter_initializer=box_initializer_segm,
            filter_optimizer=box_optimizer_segm,
    ...
    net = RTSNet(
        ...
        box_label_decoder=box_label_decoder,
        box_target_model=box_target_model,
        box_target_model_segm=box_target_model_segm,
        bbox_encoder=bbox_encoder,
        segm_encoder=segm_encoder,
        fusion_module=fusion_module,
    )
