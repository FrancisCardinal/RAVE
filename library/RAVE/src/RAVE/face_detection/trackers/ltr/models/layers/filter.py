...
def apply_filter(feat, filter, dilation_factors=None):
    ...
    scores = F.conv2d(
        feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]), filter, padding=padding, groups=num_sequences
    )
...
def _apply_filter_ksz1(feat, filter):
    ...
    scores = torch.matmul(
        filter.reshape(num_sequences, 1, 1, num_channels), feat.reshape(num_sequences, num_images, num_channels, -1)
    )
...
def _apply_feat_transpose_v1(feat, input, filter_ksz):
    ...
    filter_grad = F.conv_transpose2d(
        input.flip((2, 3)).view(1, -1, input.shape[-2], input.shape[-1]),
...
def _apply_feat_transpose_v2(feat, input, filter_ksz):
    ...
    if multiple_filters:
        filter_grad = F.conv2d(
            input.reshape(-1, num_filters, input.shape[-2], input.shape[-1]).permute(1, 0, 2, 3),
        ...
        if num_images == 1:
            return (
                filter_grad.view(num_filters, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1])
                .flip((3, 4))
                .permute(1, 0, 2, 3, 4)
            )
        return (
            filter_grad.view(num_filters, num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1])
            .sum(dim=1)
            .flip((3, 4))
            .permute(1, 0, 2, 3, 4)
        )
    ...
    return (
        filter_grad.view(num_images, num_sequences, -1, filter_grad.shape[-2], filter_grad.shape[-1])
        .sum(dim=0)
        .flip((2, 3))
    )
...
def _apply_feat_transpose_v3(feat, input, filter_ksz):
    ...
    filter_grad = F.conv2d(
        feat.reshape(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]).permute(1, 0, 2, 3),
        input.reshape(-1, 1, input.shape[-2], input.shape[-1]),
        padding=trans_pad,
        groups=num_images * num_sequences,
    )
    ...
    if multiple_filters:
        if num_images == 1:
            return filter_grad.view(
                -1, num_sequences, num_filters, filter_grad.shape[-2], filter_grad.shape[-1]
            ).permute(1, 2, 0, 3, 4)
        return (
            filter_grad.view(-1, num_images, num_sequences, num_filters, filter_grad.shape[-2], filter_grad.shape[-1])
            .sum(dim=1)
            .permute(1, 2, 0, 3, 4)
        )
    ...
    return (
        filter_grad.view(-1, num_images, num_sequences, filter_grad.shape[-2], filter_grad.shape[-1])
        .sum(dim=1)
        .permute(1, 0, 2, 3)
    )
...
def _apply_feat_transpose_v4(feat, input, filter_ksz):
    ...
    filter_grad = F.conv2d(
        feat.permute(2, 1, 0, 3, 4).reshape(feat.shape[-3], -1, feat.shape[-2], feat.shape[-1]),
