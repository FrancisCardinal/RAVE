...
class MobileNetV3(nn.Module):
    def __init__(
        self, model_mode="LARGE", num_classes=1000, multiplier=1.0, dropout_rate=0.0, output_layers=["default"]
    ):
        ...
        if model_mode == "LARGE":
            ...
            self.layer2 = nn.Sequential(
                MobileBlock(16, 24, 3, 2, "RE", False, 64), MobileBlock(24, 24, 3, 1, "RE", False, 72)
            )
            ...
            self.layer5 = nn.Sequential(
                MobileBlock(80, 112, 3, 1, "HS", True, 480), MobileBlock(112, 112, 3, 1, "HS", True, 672)
            )
...
def mobilenet3(output_layers=None, path=None):
    ...
    model = MobileNetV3(
        model_mode="LARGE", num_classes=100, multiplier=1.0, output_layers=output_layers, dropout_rate=0.8
    )
    if path is not None:
        print(path)
