...
class LWTLBoxNet(nn.Module):
    ...
    def forward(self, train_imgs, test_imgs, train_masks, test_masks, bb_train, num_refinement_iter=2):
        ...
        train_feat = self.extract_backbone_features(
            train_imgs.contiguous().view(-1, train_imgs.shape[-3], train_imgs.shape[-2], train_imgs.shape[-1])
        )
        ...
        box_mask_pred, decoder_feat = self.decoder(
            bb_mask_enc, test_feat, test_imgs.shape[-2:], ("layer4_dec", "layer3_dec", "layer2_dec", "layer1_dec")
        )
    ...
    def segment_target(self, target_filter, test_feat_tm, test_feat):
        ...
        mask_pred, decoder_feat = self.decoder(
            mask_encoding_pred, test_feat, (test_feat_tm.shape[-2] * 16, test_feat_tm.shape[-1] * 16)
        )
...
def steepest_descent_resnet50(
    ...
    if box_label_encoder_type == "ResidualDS16FeatSWBoxCatMultiBlock":
        box_label_encoder = seg_label_encoder.ResidualDS16FeatSWBoxCatMultiBlock(
...
@model_constructor
def steepest_descent_resnet50_from_checkpoint(
    net=None,
    num_filters=1,
    out_feature_dim=512,
    target_model_input_layer="layer3",
