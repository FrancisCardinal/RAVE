...
class STANet(nn.Module):
    def __init__(
        ...
        self.target_model_input_layer = (
            (target_model_input_layer,) if isinstance(target_model_input_layer, str) else target_model_input_layer
        )
    ...
    def segment_target_add_bbox_encoder(self, bbox_mask, target_filter, test_feat_clf, test_feat, segm):
        ...
        mask_pred, decoder_feat = self.decoder(
            target_scores, test_feat, (test_feat_clf.shape[-2] * 16, test_feat_clf.shape[-1] * 16)
        )
        # Output is 1, 1, h, w
        return mask_pred
...
def steepest_descent_resnet50(
    ...
    initializer_segm = seg_initializer.FilterInitializerZero(
        filter_size=filter_size, num_filters=num_filters, feature_dim=out_feature_dim, filter_groups=filter_groups
    )

    # Few-shot label generator and weight predictor
    label_encoder = seg_label_encoder.ResidualDS16FeatSWBox(
        layer_dims=label_encoder_dims + (num_filters,), feat_dim=out_feature_dim, use_final_relu=True, use_gauss=False
    )
    bbox_encoder = seg_label_encoder.ResidualDS16FeatSWBox(
        layer_dims=label_encoder_dims + (num_filters,), feat_dim=out_feature_dim, use_final_relu=True, use_gauss=False
    )
    segm_encoder = seg_label_encoder.ResidualDS16SW(
        layer_dims=label_encoder_dims[:-1] + (num_filters,), use_bn=use_bn_in_label_enc
    )
    ...
    optimizer_segm = steepestdescent.GNSteepestDescent(
        residual_module=residual_module_segm,
        num_iter=optim_iter,
        detach_length=detach_length,
        residual_batch_dim=1,
        compute_losses=True,
    ...
    target_model_segm = target_clf.LinearFilter(
        filter_size=filter_size,
        filter_initializer=initializer_segm,
        filter_optimizer=optimizer_segm,
    ...
    net = STANet(
        ...
        target_model_segm=target_model_segm,
        decoder=decoder,
