...
class CostVolume(nn.Module):
    def __init__(self, kernel_size, max_displacement, stride=1, abs_coordinate_output=False):
        ...
        self.correlation_layer = SpatialCorrelationSampler(
            kernel_size, 2 * max_displacement + 1, stride, int((kernel_size - 1) / 2)
        )
...
def remap_cost_volume(cost_volume):
    ...
    cost_volume_remapped = torch.zeros(
        (batch_size, num_rows, num_cols, num_rows, num_cols), dtype=cost_volume.dtype, device=cost_volume.device
    )
    ...
    for r in range(num_rows):
        for c in range(num_cols):
            ...
            cost_volume_remapped[:, r1_ + r1_pad_ : r2_ - r2_pad_, c1_ + c1_pad_ : c2_ - c2_pad_, r, c] = cost_volume[
                :, r1_pad_ : d_ - r2_pad_, c1_pad_ : d_ - c2_pad_, r, c
            ]
