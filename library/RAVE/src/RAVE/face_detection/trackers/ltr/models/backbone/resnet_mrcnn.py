...
def conv3x3(in_planes, out_planes, stride=1, dilation=1):
    ...
    return nn.Conv2d(
        in_planes, out_planes, kernel_size=3, stride=stride, padding=dilation, bias=False, dilation=dilation
    )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride_1x1=1, stride_3x3=1, downsample=None, dilation=1):
        ...
        self.conv2 = nn.Conv2d(
            planes, planes, kernel_size=3, stride=stride_3x3, padding=dilation, bias=False, dilation=dilation
        )
...
class ResNet(Backbone):
    ...
    def __init__(
        ...
        self.layer1 = self._make_layer(
            block, inplanes, layers[0], stride=stride[0], dilation=max(dilation_factor // 8, 1)
        )
        self.layer2 = self._make_layer(
            block, inplanes * 2, layers[1], stride=stride[1], dilation=max(dilation_factor // 4, 1)
        )
        self.layer3 = self._make_layer(
            block, inplanes * 4, layers[2], stride=stride[2], dilation=max(dilation_factor // 2, 1)
        )
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, stride_in_1x1=True):
        ...
        if self.inplanes != planes * block.expansion:
            ...
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=down_stride, bias=False),
