...
class KYSNet(nn.Module):
    ...
    def __init__(
        ...
        self.output_layers = sorted(
            list(set([self.classification_layer] + self.bb_regressor_layer + self.motion_layer))
        )
    ...
    def train_classifier(self, train_imgs, train_bb):
        ...
        train_feat_clf = train_feat_clf.view(
            num_train_images,
            num_sequences,
            train_feat_clf.shape[-3],
            train_feat_clf.shape[-2],
            train_feat_clf.shape[-1],
        )
    ...
    def extract_features(self, im, layers):
        ...
        backbone_layers = sorted(
            list(set([l for l in layers + [self.classification_layer] if l != "classification" and l != "motion"]))
        )
        all_feat = self.backbone_feature_extractor(im, backbone_layers)
        all_feat["classification"] = self.dimp_classifier.extract_classification_feat(
            all_feat[self.classification_layer]
        )
...
def kysnet_res50(
    ...
    initializer = clf_initializer.FilterInitializerLinear(
        filter_size=filter_size, filter_norm=init_filter_norm, feature_dim=appearance_feature_dim
    )
    ...
    bb_regressor = bbmodels.AtomIoUNet(
        input_dim=(4 * 128, 4 * 256), pred_input_dim=iou_input_dim, pred_inter_dim=iou_inter_dim
    )

    cost_volume_layer = cost_volume.CostVolume(
        cv_kernel_size, cv_max_displacement, stride=cv_stride, abs_coordinate_output=True
    )
