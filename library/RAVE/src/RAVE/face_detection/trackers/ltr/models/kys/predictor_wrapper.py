...
class PredictorWrapper(nn.Module):
    ...
    def forward(self, data):
        ...
        feat_map_size = (
            torch.tensor([dimp_score_cur.shape[-1], dimp_score_cur.shape[-2]])
            .view(1, 2)
            .float()
            .to(dimp_score_cur.device)
        )
        if self.fix_coordinate_shift:
            ...
            if label_prev is not None:
                ...
                label_prev = shift_features(
                    label_prev.clone().view(-1, 1, *dimp_score_cur.shape[-2:]), shift_value
                ).view(label_prev_shape)

            dimp_score_cur = shift_features(dimp_score_cur.clone(), shift_value)

        pred_response, state_new, auxiliary_outputs = self.predictor(
            cost_volume, state_prev, dimp_score_cur, label_prev
        )
        ...
        if self.fix_coordinate_shift:
            ...
            if "is_target" in auxiliary_outputs:
                auxiliary_outputs["is_target"] = shift_features(
                    auxiliary_outputs["is_target"].view(-1, 1, *dimp_score_cur.shape[-2:]), shift_value
                ).view(score_shape)

            if "is_target_after_prop" in auxiliary_outputs:
                auxiliary_outputs["is_target_after_prop"] = shift_features(
                    auxiliary_outputs["is_target_after_prop"].view(-1, 1, *dimp_score_cur.shape[-2:]), shift_value
                ).view(score_shape)

            if "is_target_new" in auxiliary_outputs:
                auxiliary_outputs["is_target_new"] = shift_features(
                    auxiliary_outputs["is_target_new"].view(-1, 1, *dimp_score_cur.shape[-2:]), shift_value
                ).view(score_shape)
    ...
    def extract_motion_feat(self, backbone_feat):
        backbone_feat = backbone_feat.view(
            -1, backbone_feat.shape[-3], backbone_feat.shape[-2], backbone_feat.shape[-1]
        )
    ...
    def predict_response(self, data, dimp_thresh=None, output_window=None):
        ...
        pred_response, state_new, auxiliary_outputs = self.predictor(
            cost_volume, state_prev, dimp_score_cur, label_prev, dimp_thresh, output_window
        )
