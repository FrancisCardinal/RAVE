...
class NerfPositionalEncoding(nn.Module):
    ...
    def forward(self, inputs):
        out = torch.cat(
            [torch.sin(i * self.factor * math.pi * inputs) for i in self.bases]
            + [torch.cos(i * self.factor * math.pi * inputs) for i in self.bases],
            axis=-1,
        )
...
class PositionEmbeddingSine(nn.Module):
    ...
    def __init__(
        ...
        scale=None,
        sine_type="lin_sine",
        avoid_aliazing=False,
        max_spatial_resolution=None,
    ):
        super().__init__()
