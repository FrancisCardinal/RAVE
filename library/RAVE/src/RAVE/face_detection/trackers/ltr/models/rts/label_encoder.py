...
class ResidualDS16SW(nn.Module):
    ...
    def __init__(self, layer_dims, use_bn=True):
        ...
        self.label_pred = conv_block(
            layer_dims[2], layer_dims[3], kernel_size=3, stride=1, padding=1, relu=True, batch_norm=use_bn
        )
...
class ResidualDS16FeatSWBoxCatMultiBlock(nn.Module):
    def __init__(
        ...
        self.label_pred = conv_block(
            layer_dims[-2], layer_dims[-1], kernel_size=3, stride=1, padding=1, relu=use_final_relu, batch_norm=final_bn
        )
...
class ResidualDS16FeatSWBox(nn.Module):
    def __init__(self, layer_dims, feat_dim, use_final_relu=True, use_gauss=True, use_bn=False, use_sample_w=True):
        ...
        self.label_pred = conv_block(
            layer_dims[3], layer_dims[4], kernel_size=3, stride=1, padding=1, relu=use_final_relu
        )
