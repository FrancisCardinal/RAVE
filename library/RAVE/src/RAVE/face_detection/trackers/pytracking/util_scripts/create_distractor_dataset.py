...
def determine_frame_state(tracking_data, tracker, seq, th=0.25):
    ...
    if num_candidates >= 2:
        ...
        anno_and_target_candidate_score_dists = torch.sqrt(
            torch.sum(
                (tracking_data["target_anno_coord"] - tracking_data["target_candidate_coords"]) ** 2, dim=1
            ).float()
        )
        ...
        score_dist_anno_2nd_highest_score_candidate = (
            anno_and_target_candidate_score_dists[ids[1]] if num_candidates > 1 else 10000
        )
