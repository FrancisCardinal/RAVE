...
class Rotate(Transform):
    ...
    def __call__(self, image, is_mask=False):
        if isinstance(image, torch.Tensor):
            ...
            R = np.array([[math.cos(self.angle), math.sin(self.angle)], [-math.sin(self.angle), math.cos(self.angle)]])
...
class RandomAffine(Transform):
    ...
    def __call__(self, image, is_mask=False):
        ...
        if not is_mask:
            image_t = cv.warpAffine(image, t_mat, output_sz, flags=cv.INTER_LINEAR, borderMode=self.border_flag)
        else:
            image_t = cv.warpAffine(image, t_mat, output_sz, flags=cv.INTER_NEAREST, borderMode=self.border_flag)
