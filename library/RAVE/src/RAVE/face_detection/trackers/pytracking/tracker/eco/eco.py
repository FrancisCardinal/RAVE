...
class ECO(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.reg_filter = TensorList(
            [
                dcf.get_reg_filter(self.img_support_sz, self.base_target_sz, fparams).to(self.params.device)
        ...
        self.projection_matrix = TensorList(
            [torch.svd(C)[0][:, :cdim].clone() for C, cdim in zip(cov_x, self.compressed_dim)]
        )
        ...
        self.training_samples = TensorList(
            [
                xf.new_zeros(xf.shape[2], xf.shape[3], self.params.sample_memory_size, cdim, 2)
        ...
        self.joint_optimizer = GaussNewtonCG(
            self.joint_problem, joint_var, debug=(self.params.debug >= 1), visdom=self.visdom
        )
            ...
            for sfe, a, b in zip(sf, alpha, beta):
                ...
                for sind, sz in enumerate(sample_sz):
                    ...
                    scores_scales.append(
                        F.pad(
                            fourier.sample_fs(sfe[sind : sind + 1, ...], sz),
                            (
                                math.floor(pd[1].item()),
                                math.ceil(pd[1].item()),
                                math.floor(pd[0].item()),
                                math.ceil(pd[0].item()),
                            ),
                        )
                    )
    ...
    def update_sample_weights(self):
        replace_ind = []
        for sw, prev_ind, num_samp, fparams in zip(
            self.sample_weights, self.previous_replace_ind, self.num_stored_samples, self.fparams
        ):
