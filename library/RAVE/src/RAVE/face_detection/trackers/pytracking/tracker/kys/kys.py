...
class KYS(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        self.sigma = (
            (self.label_sz / self.img_support_sz * self.base_target_sz).prod().sqrt()
            * output_sigma_factor
            * torch.ones(2)
        )
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if flag != "not_found":
            if self.params.get("use_iou_net", True):
                ...
                update_scale_flag = update_scale_flag and (
                    dimp_score_at_loc > self.params.get("min_dimp_score_for_scale_update", -1.0)
                )
        ...
        if flag != "not_found":
            ...
            prev_label = self.get_label_function(sample_pos[scale_ind, :], sample_scales[scale_ind]).to(
                self.params.device
            )[0]
            self.prev_state_handler.set_data(
                self.frame_num, motion_feat, new_state_vector, test_patch, new_state, prev_label, box_patch
            )
            self.prev_anno = self.get_iounet_box(
                self.pos, self.target_sz, sample_pos[scale_ind, :], sample_scales[scale_ind]
            )
    ...
    def init_motion_module(self, im):
        ...
        self.prev_state_handler.set_data(0, motion_feat, None, test_patch, current_bb, prev_label, box_patch)
    ...
    def get_response_prediction(self, backbone_feat, scores_dimp):
        ...
        (
            prev_motion_feat,
            prev_motion_state_vector,
            prev_test_patch,
            prev_label,
            box_patch,
        ) = self.prev_state_handler.get_data()
        ...
        if (
            prev_motion_state_vector is not None
            and self.params.get("move_feat_to_center", False)
            and not ((box_c < box_c_max.to(box_c.device)) * (box_c > box_c_min.to(box_c.device))).all()
        ):
            ...
            prev_motion_state_vector = CenterShiftFeatures(feature_stride=16)(
                prev_motion_state_vector.clone(), box_patch
            )
        ...
        with torch.no_grad():
            resp_output = self.net.predictor.predict_response(
                predictor_input, dimp_thresh=self.params.get("dimp_threshold", None), output_window=window_fn
            )
    ...
    def perform_hn_mining_dimp(self, score_dimp, max_disp1, sample_scales):
        ...
        target_neigh_sz = (
            self.params.target_neighborhood_scale_safe
    ...
    def update_sample_weights(
        self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, learning_rate=None
    ):
        # Update weights and get index to replace
        replace_ind = []
        for sw, prev_ind, num_samp, num_init in zip(
            sample_weights, previous_replace_ind, num_stored_samples, num_init_samples
        ):
            lr = learning_rate
            if lr is None:
        ...
        if self.params.get("window_output", False):
            ...
            if self.params.get("use_clipped_window", False):
                self.output_window = dcf.hann2d_clipped(
                    score_map_sz.long(),
                    (score_map_sz * self.params.effective_search_area / self.params.search_area_scale).long(),
                    centered=True,
            ...
            if self.params.get("windom_clamp_factor", None) is not None:
                self.output_window = (self.output_window * (1.0 / self.params.get("windom_clamp_factor"))).clamp(
                    0.0, 1.0
                )
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        ...
        if update_scale:
            if self.params.has("target_scale_update_rate"):
                self.target_scale = new_scale * self.params.target_scale_update_rate + self.target_scale * (
                    1 - self.params.target_scale_update_rate
                )
            else:
