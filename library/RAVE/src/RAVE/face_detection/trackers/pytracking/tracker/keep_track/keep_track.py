...
class KeepTrack(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(
            im, self.get_centered_sample_pos(), self.target_scale * self.params.scale_factors, self.img_sample_sz
        )
        ...
        out = self.localize_target_by_candidate_matching(
            im_patches, backbone_feat, scores, search_area_box, sample_pos, sample_scales, im.shape[2:]
        )
        ...
        if update_flag and self.params.get("update_classifier", False):
            ...
            self.update_classifier(
                train_x, train_y, target_box, learning_rate, s[scale_ind, ...], target_label_certainty
            )
        ...
        out = {"target_bbox": output_state, "object_presence_score": object_presence_score.cpu().item()}
    ...
    def search_area_rescaling(self):
        if len(self.target_scales) > 0:
            ...
            ]  # only boxes that are bigger than the `not found`
    ...
    def get_centered_sample_pos(self):
        ...
        return self.pos + ((self.feature_sz + self.kernel_size) % 2) * self.target_scale * self.img_support_sz / (
            2 * self.feature_sz
        )
    ...
    def localize_target_by_candidate_matching(
        self, im_patches1, backbone_feat1, score_map1, search_area_box1, sample_pos, sample_scales, img_shape
    ):
        ...
        if self.previous_candidates is None or (self.frame_num - self.previous_candidates["frame_num"]) > 1:
            ...
            if (
                self.previous_candidates["scores"].shape[0] == 1
                and current_candidates["scores"].shape[0] == 1
                and self.previous_candidates["scores"].max() > 0.5
                and current_candidates["scores"].max() > 0.5
            ):
                match_preds = {"matches1": torch.zeros(1).long(), "match_scores1": torch.ones(1)}

            else:
                match_preds = self.extract_matches(
                    descriptors0=self.previous_candidates["descriptors"],
                ...
                if self.visdom is not None:
                    matching_visualization_data = dict(
                        match_preds=match_preds,
                        im_patches0=self.previous_im_patches,
                        im_patches1=im_patches1,
                        score_map0=self.previous_score_map,
                        score_map1=score_map1,
                        frameid0=self.previous_candidates["frame_num"],
                        frameid1=self.frame_num,
                    )

            self.candidate_collection.update(
            ...
            candidate_coord = current_candidates["tsm_coords"][
                self.candidate_collection.candidate_id_of_selected_candidate
            ]
        ...
        if self.candidate_collection is None:
            self.candidate_collection = CandidateCollection(
                scores=current_candidates["scores"],
                tsm_coords=current_candidates["tsm_coords"],
                candidate_selection_is_certain=(self.frame_num < 10),
            )

        self.previous_candidates = dict(
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        target_neigh_sz = (
            self.params.target_neighborhood_scale * (self.target_sz / sample_scale) * (output_sz / self.img_support_sz)
        )
        ...
        if (
            max_score2 > self.params.hard_negative_threshold * max_score1
            and max_score2 > self.params.target_not_found_threshold
        ):
            return translation_vec1, scale_ind, scores_hn, "hard_negative"
    ...
    def extract_descriptors_and_keypoints(self, backbone_feat, score_map, search_area_box):
        ...
        with torch.no_grad():
            descriptors = self.target_candidate_matching_net.descriptor_extractor.get_descriptors(
                frame_feat_clf, tsm_coords
            )
    ...
    def extract_matches(self, descriptors0, descriptors1, img_coords0, img_coords1, scores0, scores1, image_shape):
        data = {
    ...
    def extract_backbone_features(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        im_patches, patch_coords = sample_patch_multiscale(
            im,
            pos,
            scales,
            sz,
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if random_shift_factor > 0:
            get_rand_shift = (
                lambda: ((torch.rand(2) - 0.5) * self.img_sample_sz * random_shift_factor + global_shift)
                .long()
                .tolist()
            )
        ...
        if "shift" in augs:
            self.transforms.extend(
                [
                    augmentation.Translation(shift, aug_output_sz, global_shift.long().tolist())
        ...
        if "relativeshift" in augs:
            ...
            self.transforms.extend(
                [
                    augmentation.Translation(get_absolute(shift), aug_output_sz, global_shift.long().tolist())
        ...
        im_patches = sample_patch_transformed(
    ...
    def init_target_label_certainties(self, train_x: TensorList):
        ...
        self.target_label_certainties[:num_train_samples] = 1.0
    ...
    def init_target_labels(self, train_x: TensorList):
        ...
        self.sigma = (
            (self.feature_sz / self.img_support_sz * self.base_target_sz).prod().sqrt()
            * output_sigma_factor
            * torch.ones(2)
        )
        ...
        for target, x in zip(self.target_labels, train_x):
            ...
            for i, T in enumerate(self.transforms[: x.shape[0]]):
                ...
                target[i, 0, ...] = dcf.label_function_spatial(
                    self.feature_sz, self.sigma, sample_center, end_pad=ksz_even
                )
    ...
    def update_memory(
        self, sample_x: TensorList, sample_y: TensorList, target_box, learning_rate=None, target_label_certainty=None
    ):
        # Update weights and get replace ind
        if self.candidate_collection is None or self.candidate_collection.object_id_of_selected_candidate == 0:
            target_label_certainty = torch.max(
                target_label_certainty, torch.sqrt(target_label_certainty.clone().detach())
            )

        certainties = [self.target_label_certainties.view(-1) * self.sample_weights[0].view(-1)]

        replace_ind = self.update_sample_weights_based_on_certainty(
            certainties,
            self.sample_weights,
    ...
    def update_sample_weights_based_on_certainty(
        self,
        certainties,
        sample_weights,
        ...
        replace_ind = []
        for cert, sw, prev_ind, num_samp, num_init in zip(
            certainties, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples
        ):
            lr = learning_rate
            if lr is None:
    ...
    def init_iou_net(self, backbone_feat):
        ...
        if self.params.iounet_augmentation:
            ...
            target_boxes.append(
                ...
                + torch.Tensor([self.transforms[0].shift[1], self.transforms[0].shift[0], 0, 0])
            )
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        if hasattr(self.net, "classifier"):
            pred_module = getattr(
                self.net.classifier.filter_optimizer, "score_predictor", self.net.classifier.filter_optimizer
            )
        elif hasattr(self.net, "dimp_classifier"):
            self.net.classifier = self.net.dimp_classifier
            pred_module = getattr(
                self.net.dimp_classifier.filter_optimizer, "score_predictor", self.net.dimp_classifier.filter_optimizer
            )
        else:
        ...
        if plot_loss:
            ...
            if self.visdom is not None:
                self.visdom.register(
                    (self.losses, torch.arange(self.losses.numel())), "lineplot", 3, "Training Loss" + self.id_str
                )
            elif self.params.debug >= 3:
                plot_graph(self.losses, 10, title="Training Loss" + self.id_str)

    def update_classifier(
        self, train_x, train_y, target_box, learning_rate=None, scores=None, target_label_certainty=None
    ):
        if target_label_certainty is None:
            target_label_certainty = 1.0
        ...
        if num_iter > 0:
            ...
            with torch.no_grad():
                target_filter, _, losses = self.net.classifier.filter_optimizer(
                    TensorList([self.target_filter]),
                    num_iter=num_iter,
                    feat=samples,
                    bb=target_boxes,
    ...
    def optimize_boxes_default(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            step_length = torch.Tensor(
                [step_length[0], step_length[0], step_length[1], step_length[1]], device=self.params.device
            ).view(1, 1, 4)
    ...
    def optimize_boxes_relative(self, iou_features, init_boxes):
        ...
        if isinstance(step_length, (tuple, list)):
            step_length = (
                torch.Tensor([step_length[0], step_length[0], step_length[1], step_length[1]])
                .to(self.params.device)
                .view(1, 1, 4)
            )
    ...
    def visualize_candidate_matching(self, data):
        ...
        def add_matching_probability(axis, prob, id0, id1, num_entry, offset, color):
            ...
            y = num_entry * 20
            axis.text(
                x, y, "{}--[{:0.1f}]--{}".format(id0, 100 * prob, id1), ha="left", va="center", color=color, fontsize=10
            )
        ...
        if data is not None:
            ...
            im_patches = torch.cat(
                [data["im_patches0"], 255 * torch.ones((1, 3, data["im_patches0"].shape[3], gap)), data["im_patches1"]],
                dim=3,
            ).permute(0, 2, 3, 1)
            ...
            for i, m in enumerate(matches1):
                ...
                if m >= 0:
                    ...
                    add_matching_probability(
                        ax,
                        prob=assignment_probs[m, i],
                        id0=m,
                        id1=i,
                        offset=2 * img_sz + gap,
                        num_entry=sum(torch.tensor(matches1[:i]) >= 0),
                        color=color,
                    )
