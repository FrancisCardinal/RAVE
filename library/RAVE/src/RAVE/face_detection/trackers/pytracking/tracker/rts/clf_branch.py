...
class ClassifierBranch:
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if random_shift_factor > 0:
            get_rand_shift = (
                lambda: ((torch.rand(2) - 0.5) * self.clf_img_sample_sz * random_shift_factor + global_shift)
                .long()
                .tolist()
            )
        ...
        im_patches = sample_patch_transformed(
            im, self.clf_init_sample_pos, self.clf_init_sample_scale, aug_expansion_sz, self.clf_transforms
        )
    ...
    def update_state(
        self, track_test_x, clf_scores, sample_coords, lwl_pos, lwl_scale, lwl_size, is_lost_seg, is_seg_too_small
    ):
        ...
        clf_new_state = torch.cat(
            (self.clf_pos[[1, 0]] - (self.clf_target_sz[[1, 0]] - 1) / 2, self.clf_target_sz[[1, 0]])
        )
    ...
    def update_classifier_model(self, track_test_x, sample_coords, scale_ind, s, flag):
        ...
        if update_flag and self.params.get("update_classifier", False):
            ...
            target_box = self.get_iounet_box(
                self.clf_pos, self.clf_target_sz, clf_sample_pos[scale_ind, :], clf_sample_scale[scale_ind]
            )

            train_y = self.get_label_function(
                self.clf_pos, clf_sample_pos[scale_ind, :], clf_sample_scale[scale_ind]
    ...
    def update_classifier_memory(self, sample_x: TensorList, sample_y: TensorList, target_box, learning_rate=None):
        ...
        replace_ind = self.update_classifier_sample_weights(
            self.clf_sample_weights,
            self.clf_previous_replace_ind,
            self.clf_num_stored_samples,
            self.clf_num_init_samples,
            learning_rate,
        )
    ...
    def update_classifier_sample_weights(
    ...
    def init_target_labels(self, train_x: TensorList):
        self.clf_target_labels = TensorList(
            [
                x.new_zeros(
                    self.params.clf_sample_memory_size,
                    1,
        ...
        self.clf_sigma = (
            (self.clf_feature_sz / self.clf_img_support_sz * self.clf_base_target_sz).prod().sqrt()
            * output_sigma_factor
            * torch.ones(2)
        )

        # Center pos in normalized img_coords
        target_center_norm = (self.clf_pos - self.clf_init_sample_pos) / (
            self.clf_init_sample_scale * self.clf_img_support_sz
        )
        ...
        for target, x in zip(self.clf_target_labels, train_x):
            ...
            for i, T in enumerate(self.clf_transforms[: x.shape[0]]):
                ...
                target[i, 0, ...] = dcf.label_function_spatial(
                    self.clf_feature_sz, self.clf_sigma, sample_center, end_pad=ksz_even
                )
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales):
        ...
        target_neigh_sz = (
            self.params.clf_target_neighborhood_scale
            * (self.clf_target_sz / sample_scale)
            * (output_sz / self.clf_img_support_sz)
        )
        ...
        prev_target_vec = (self.clf_pos - sample_pos[scale_ind, :]) / (
            (self.clf_img_support_sz / output_sz) * sample_scale
        )
        ...
        if max_score2 > self.params.clf_distractor_threshold * max_score1:
            ...
            return translation_vec1, scale_ind, scores_hn, "uncertain"
        if (
            max_score2 > self.params.clf_hard_negative_threshold * max_score1
            and max_score2 > self.params.clf_target_not_found_threshold
        ):
            return translation_vec1, scale_ind, scores_hn, "hard_negative"
