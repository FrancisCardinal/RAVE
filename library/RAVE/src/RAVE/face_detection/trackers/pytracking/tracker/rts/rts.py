...
class RTS(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        safety_cond = (
            (not safe_updating)
            or (safe_updating and strict_force_seg and force_seg_train)
            or (safe_updating and not self.is_lost_seg and not self.is_lost_clf)
        )
        ...
        if self.frame_num > 2 and update_target_model:
            ...
            if self.frame_num % self.params.get("train_sample_interval", 1) == 0 or force_seg_train:
                self.update_memory(
                    TensorList([self.prev_segm_test_x]), seg_prob_crop.clone(), self.params.learning_rate
                )
        ...
        segmentation_scores, mask_encoding_pred = self.segment_target(segm_test_x, backbone_feat, encoded_clf_scores)
        ...
        trust_clf_always = self.params.get("trust_clf_always", False)
        if trust_clf_always or (
            (self.is_lost_seg or self.seg_too_small) and self.params.get("use_clf_box_when_no_seg", True)
        ):
    ...
    def merge_results(self, out_all):
        ...
        out_merged["segmentation_raw"] = OrderedDict(
            {key: segmentation_maps_np_agg[i + 1] for i, key in enumerate(obj_ids)}
        )
    ...
    def get_target_state(self, segmentation_prob_im):
        ...
        if self.params.get("seg_to_bb_mode") == "var":
            ...
            e_y = (
                torch.sum(
                    segmentation_prob_im.sum(dim=-1) * torch.arange(segmentation_prob_im.shape[-2], dtype=torch.float32)
                )
                / prob_sum
            )
            e_x = (
                torch.sum(
                    segmentation_prob_im.sum(dim=-2) * torch.arange(segmentation_prob_im.shape[-1], dtype=torch.float32)
                )
                / prob_sum
            )
            ...
            e_h = (
                torch.sum(
                    segmentation_prob_im.sum(dim=-1)
                    * (torch.arange(segmentation_prob_im.shape[-2], dtype=torch.float32) - e_y) ** 2
                )
                / prob_sum
            )
            e_w = (
                torch.sum(
                    segmentation_prob_im.sum(dim=-2)
                    * (torch.arange(segmentation_prob_im.shape[-1], dtype=torch.float32) - e_x) ** 2
                )
                / prob_sum
            )
    ...
    def convert_seg_scores_crop_to_image(self, segmentation_scores, im, sample_coords):
        ...
        segmentation_scores_im[r1 + r1_pad : r2 - r2_pad, c1 + c1_pad : c2 - c2_pad] = segmentation_scores_re[
            r1_pad : shape[0] - r2_pad, c1_pad : shape[1] - c2_pad
        ]
    ...
    def segment_target(self, sample_tm_feat, sample_x, encoded_clf_scores):
        with torch.no_grad():
            segmentation_scores, mask_encoding_pred = self.net.segment_target(
                self.target_filter, sample_tm_feat, sample_x, encoded_clf_scores
            )
    ...
    def generate_init_samples(self, im: torch.Tensor, init_mask):
        ...
        init_masks = sample_patch_transformed(
            init_mask, self.init_sample_pos, self.init_sample_scale, aug_expansion_sz, self.transforms, is_mask=True
        )
    ...
    def init_memory(self, train_x: TensorList, masks):
        ...
        self.target_masks = masks.new_zeros(
            self.params.sample_memory_size, masks.shape[-3], masks.shape[-2], masks.shape[-1]
        )
    ...
    def init_target_model(self, init_backbone_feat, init_masks):
        ...
        with torch.no_grad():
            self.target_filter, _, losses = self.net.target_model.get_filter(
                x.unsqueeze(1), few_shot_label, few_shot_sw, num_iter=num_iter
            )
