...
class ToMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        if (
            update_flag
            and self.params.get("update_classifier", False)
            and scores_raw.max() > self.params.get("conf_ths", 0.0)
        ):
            # Get train sample
    ...
    def direct_bbox_regression(self, bbox_preds, sample_coords, score_loc, scores_raw):
        ...
        x1 = (
            xs1[sl[0], sl[1]] / self.img_sample_sz[1] * (sample_coords[0, 3] - sample_coords[0, 1])
            + sample_coords[0, 1]
        )
        y1 = (
            ys1[sl[0], sl[1]] / self.img_sample_sz[0] * (sample_coords[0, 2] - sample_coords[0, 0])
            + sample_coords[0, 0]
        )
        x2 = (
            xs2[sl[0], sl[1]] / self.img_sample_sz[1] * (sample_coords[0, 3] - sample_coords[0, 1])
            + sample_coords[0, 1]
        )
        y2 = (
            ys2[sl[0], sl[1]] / self.img_sample_sz[0] * (sample_coords[0, 2] - sample_coords[0, 0])
            + sample_coords[0, 0]
        )
        w = x2 - x1
        h = y2 - y1
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            ...
            (
                ...
                cls_test_feat_enc,
                bbreg_test_feat_enc,
            ) = self.net.head.get_filter_and_features_in_parallel(
                train_feat,
                test_feat,
