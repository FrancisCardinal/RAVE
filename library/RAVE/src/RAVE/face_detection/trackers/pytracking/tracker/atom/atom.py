...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        if self.params.get("search_area_shape", "square") == "square":
            self.img_sample_sz = torch.round(
                torch.sqrt(torch.prod(self.base_target_sz * self.params.search_area_scale))
            ) * torch.ones(2)
    ...
    def init_optimization(self, train_x, init_y):
        ...
        self.params.update_projection_matrix = (
            self.params.get("update_projection_matrix", True) and self.params.use_projection_matrix
        )
        ...
        if self.params.update_projection_matrix:
            self.joint_problem = FactorizedConvProblem(
                ...
                init_y,
                self.filter_reg,
                self.fparams.attribute("projection_reg"),
                self.params,
            ...
            if analyze_convergence:
                ...
                for val_name, values in zip(
                    ["loss", "gradient"], [self.joint_optimizer.losses, self.joint_optimizer.gradient_mags]
                ):
        ...
        # Initialize optimizer
        self.conv_problem = ConvProblem(
            self.training_samples, self.y, self.filter_reg, self.sample_weights, self.response_activation
        )
    ...
    def localize_target(self, scores_raw):
        ...
        for i, (sz, ksz) in enumerate(zip(self.feature_sz, self.kernel_size)):
            sf_weighted[i] = fourier.shift_fs(
                sf_weighted[i], math.pi * (1 - torch.Tensor([ksz[0] % 2, ksz[1] % 2]) / sz)
            )
    ...
    def localize_advanced(self, scores):
        ...
        if self.output_window is not None and self.params.get("perform_hn_without_windowing", False):
            ...
            scores_orig = torch.cat(
                [scores_orig[..., (sz[0] + 1) // 2 :, :], scores_orig[..., : (sz[0] + 1) // 2, :]], -2
            )
            scores_orig = torch.cat(
                [scores_orig[..., :, (sz[1] + 1) // 2 :], scores_orig[..., :, : (sz[1] + 1) // 2]], -1
            )
    ...
    def extract_processed_sample(
        self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor
            ...
            for i, use_aug in enumerate(self.fparams.attribute("use_augmentation")):
                if use_aug:
                    init_samples[i] = torch.cat(
                        [
                            init_samples[i],
                            F.dropout2d(init_samples[i][0:1, ...].expand(num, -1, -1, -1), p=prob, training=True),
                        ]
                    )
        ...
        if self.params.use_projection_matrix:
            ...
            if proj_init_method == "pca":
                ...
                self.projection_matrix = TensorList(
                    [
                        None if cdim is None else torch.svd(C)[0][:, :cdim].t().unsqueeze(-1).unsqueeze(-1).clone()
                ...
                self.projection_matrix = TensorList(
                    [
                        None
                        if cdim is None
                        else ex.new_zeros(cdim, ex.shape[1], 1, 1).normal_(0, 1 / math.sqrt(ex.shape[1]))
    ...
    def init_memory(self, train_x):
        ...
        self.training_samples = TensorList(
            [
                x.new_zeros(self.params.sample_memory_size, cdim, x.shape[2], x.shape[3])
    ...
    def update_sample_weights(
        self, sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, fparams, learning_rate=None
    ):
        # Update weights and get index to replace in memory
        replace_ind = []
        for sw, prev_ind, num_samp, num_init, fpar in zip(
            sample_weights, previous_replace_ind, num_stored_samples, num_init_samples, fparams
        ):
            lr = learning_rate
            if lr is None:
    ...
    def optimize_boxes(self, iou_features, init_boxes):
        ...
        if box_refinement_space == "default":
            ...
            for i_ in range(self.params.box_refinement_iter):
                ...
                # Update proposal
                step = (
                    update_mask_float * step_length * bb_init.grad * bb_init[:, :, 2:].repeat(1, 1, 2)
