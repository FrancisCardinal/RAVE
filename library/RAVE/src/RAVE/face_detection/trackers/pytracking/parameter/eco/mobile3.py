...
def parameters():
    ...
    params.init_GN_iter = (
        10  # The number of Gauss-Newton iterations used in the first frame (only if the projection matrix is updated)
    )
    params.post_init_CG_iter = 0  # CG iterations to run after GN
    params.fletcher_reeves = (
        False  # Use the Fletcher-Reeves (true) or Polak-Ribiere (false) formula in the Conjugate Gradient
    )
    ...
    shallow_params.reg_sparsity_threshold = (
        0.05  # A relative threshold of which DFT coefficients that should be set to zero
    )
    ...
    deep_params.reg_sparsity_threshold = (
        0.1  # A relative threshold of which DFT coefficients that should be set to zero
    )

    fparams = FeatureParams(feature_params=[shallow_params, deep_params])
    features = deep.Mobilenet(
        output_layers=["init_conv", "layer5"],
