...
import importlib
from RAVE.face_detection.trackers.pytracking.evaluation.data import SequenceList
...
dataset_dict = dict(
    ...
    got10kvos_val=DatasetInfo(
        module=pt % "got10k", class_name="GOT10KDataset", kwargs=dict(split="val", vos_mode=True)
    ),
    ...
    lasot_extension_subset=DatasetInfo(
        module=pt % "lasotextensionsubset", class_name="LaSOTExtensionSubsetDataset", kwargs=dict()
    ),
    ...
    dv2017_test_dev=DatasetInfo(
        module="ltr.dataset.davis", class_name="Davis", kwargs=dict(version="2017", split="test-dev")
    ),
    dv2017_test_chal=DatasetInfo(
        module="ltr.dataset.davis", class_name="Davis", kwargs=dict(version="2017", split="test-challenge")
    ),
    yt2019_test=DatasetInfo(
        module="ltr.dataset.youtubevos", class_name="YouTubeVOS", kwargs=dict(version="2019", split="test")
    ),
    yt2019_valid=DatasetInfo(
        module="ltr.dataset.youtubevos", class_name="YouTubeVOS", kwargs=dict(version="2019", split="valid")
    ),
    yt2019_valid_all=DatasetInfo(
        module="ltr.dataset.youtubevos",
        class_name="YouTubeVOS",
        kwargs=dict(version="2019", split="valid", all_frames=True),
    ),
    yt2018_valid_all=DatasetInfo(
        module="ltr.dataset.youtubevos",
        class_name="YouTubeVOS",
        kwargs=dict(version="2018", split="valid", all_frames=True),
    ),
    yt2018_jjval=DatasetInfo(
        module="ltr.dataset.youtubevos", class_name="YouTubeVOS", kwargs=dict(version="2018", split="jjvalid")
    ),
    yt2019_jjval=DatasetInfo(
        module="ltr.dataset.youtubevos",
        class_name="YouTubeVOS",
        kwargs=dict(version="2019", split="jjvalid", cleanup=["starts"]),
    ),
    yt2019_jjval_all=DatasetInfo(
        module="ltr.dataset.youtubevos",
        class_name="YouTubeVOS",
