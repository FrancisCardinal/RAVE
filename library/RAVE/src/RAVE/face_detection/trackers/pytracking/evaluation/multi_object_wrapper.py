...
class MultiObjectWrapper:
    ...
    def default_merge(self, out_all):
        ...
        if "segmentation" in out_types and out_first["segmentation"] is not None:
            ...
            merged_segmentation = obj_ids[
                np.where(segmentation_maps.max(axis=0) > segm_threshold, segmentation_maps.argmax(axis=0) + 1, 0)
            ]
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        for obj_id in info["init_object_ids"]:
            ...
            init_default = {
                "target_bbox": init_info_split[obj_id].get("init_bbox"),
