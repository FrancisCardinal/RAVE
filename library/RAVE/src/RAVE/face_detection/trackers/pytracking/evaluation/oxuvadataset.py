...
class OxUvADataset(BaseDataset):
    ...
    def __init__(self, split):
        ...
        dev_tasks_df = pd.read_csv(
            os.path.join(self.base_path, "tasks", "dev.csv"),
            header=None,
            names=["video_id", "object_id", "init_frame", "last_frame", "xmin", "xmax", "ymin", "ymax"],
            dtype={
                "video_id": str,
                "object_id": str,
                "init_frame": int,
        ...
        test_tasks_df = pd.read_csv(
            os.path.join(self.base_path, "tasks", "test.csv"),
            header=None,
            names=["video_id", "object_id", "init_frame", "last_frame", "xmin", "xmax", "ymin", "ymax"],
            dtype={
                "video_id": str,
        ...
        self.dev_annotations_df = pd.read_csv(
            os.path.join(self.base_path, "annotations", "dev.csv"),
            header=None,
            names=[
                "video_id",
                "object_id",
    ...
    def _construct_sequence(self, sequence_id):
        ...
        frames_list = [
            os.path.join(frames_path, "{:06d}.jpeg".format(frame_number))
            for frame_number in range(init_frame, last_frame + 1)
        ]
    ...
    def build_dev_annotations(self, frames_list, init_frame, last_frame, object_id, video_id):
        # Load first image in sequence to extract width and height to compute bbox coordinates in images.
        w, h = Image.open(frames_list[0]).size
        annotation_mask = np.logical_and(
            self.dev_annotations_df["video_id"] == video_id, self.dev_annotations_df["object_id"] == object_id
        )
