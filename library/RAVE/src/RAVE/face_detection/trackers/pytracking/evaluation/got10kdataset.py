...
class GOT10KDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        ...
        if self.vos_mode:
            ...
            masks = [
                self._load_mask(Path(self._get_anno_frame_path(seq_mask_path, f[:-3] + "png"))) for f in frame_list[0:1]
            ]

        return Sequence(sequence_name, frames_list, "got10k", ground_truth_rect.reshape(-1, 4), ground_truth_seg=masks)
