...
from RAVE.face_detection.trackers.pytracking.features.preprocessing import numpy_to_torch
from RAVE.face_detection.trackers.pytracking.utils.plotting import (
        ...
        if self.block_list is not None and self.show_data:
            for i, d in enumerate(self.block_list):
                if d["value"]:
                    ...
                    self.visdom.heatmap(self.raw_data[i, :, :].clone(), opts={"title": fig_title}, win=fig_title)
...
class VisBBReg(VisBase):
    ...
    def draw_data(self):
        ...
        final_box_image = show_image_with_boxes(
            disp_image.clone(), self.final_boxes.clone(), self.final_ious.clone(), disp_ids=ids
        )
