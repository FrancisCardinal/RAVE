...
def extract_results(
    ...
    ave_success_rate_plot_overlap = torch.zeros(
        (len(dataset), len(trackers), threshold_set_overlap.numel()), dtype=torch.float32
    )
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            ave_success_rate_plot_overlap[seq_id, trk_id, :] = (
                err_overlap.view(-1, 1) > threshold_set_overlap.view(1, -1)
            ).sum(0).float() / seq_length
            ave_success_rate_plot_center[seq_id, trk_id, :] = (
                err_center.view(-1, 1) <= threshold_set_center.view(1, -1)
            ).sum(0).float() / seq_length
            ave_success_rate_plot_center_norm[seq_id, trk_id, :] = (
    ...
    tracker_names = [
        {"name": t.name, "param": t.parameter_name, "run_id": t.run_id, "disp_name": t.display_name} for t in trackers
    ]
