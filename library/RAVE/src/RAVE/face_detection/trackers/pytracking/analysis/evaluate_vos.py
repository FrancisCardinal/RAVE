...
def evaluate_dataset(results_path, dset_name, measure="J", to_file=True, scores=False, sequences=None, quiet=False):
    ...
    for j, sequence in enumerate(dset):
        ...
        for obj_id, score in r["raw"].items():
            ...
            if n_objs > 1:
                _print(
                    "joint {obj}: acc {score:.3f} ┊{apf}┊".format(obj=obj_id, score=s, apf=utils.text_bargraph(score))
                )
        ...
        _print(
            "final  : acc {seq:.3f} ({dset:.3f}) ┊{apf}┊".format(
                seq=seq_score, dset=np.mean(dset_scores), apf=utils.text_bargraph(seq_mean_score)
            )
        )

    _print(
        "%s: %.3f, recall: %.3f, decay: %.3f"
        % (measure, utils.mean(dset_scores), utils.mean(dset_recall), utils.mean(dset_decay))
    )
...
def evaluate_vos(trackers, dataset="yt2019_jjval", force=False):
    ...
    for t in trackers:
        ...
        if os.path.exists(csv_name_global_path) and os.path.exists(csv_name_per_sequence_path) and not force:
            ...
            seq_names, dset_scores, dset_recall, dset_decay = evaluate_dataset(
                results_path, dataset, measure="J", to_file=False, scores=False, sequences=None
            )
            ...
            table_seq = pd.DataFrame(
                data=list(zip(seq_names, dset_scores, dset_recall, dset_decay)), columns=seq_measures
            )
