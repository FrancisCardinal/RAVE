...
def check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation=False, **kwargs):
    ...
    if not check_eval_data_is_valid(eval_data, trackers, dataset):
        ...
        tracker_names = [
            {"name": t.name, "param": t.parameter_name, "run_id": t.run_id, "disp_name": t.display_name}
            for t in trackers
        ]
...
def plot_results(
    trackers, dataset, report_name, merge_results=False, plot_types=("success"), force_evaluation=False, **kwargs
):
    """
    ...
    if "success" in plot_types:
        ...
        plot_draw_save(
            auc_curve, threshold_set_overlap, auc, tracker_names, plot_draw_styles, result_plot_path, success_plot_opts
        )
    ...
    for trk_id, d_name in enumerate(row_labels):
        ...
        for (score_type, score_value), s_w in zip(scores.items(), score_widths):
            report_text = "{prev} {score: <{width}} |".format(
                prev=report_text, score="{:0.2f}".format(score_value[trk_id].item()), width=s_w
            )
        report_text = "{prev}\n".format(prev=report_text)
...
def print_results(trackers, dataset, report_name, merge_results=False, plot_types=("success"), **kwargs):
...
def plot_got_success(trackers, report_name):
    ...
    for trk_id, trk in enumerate(trackers):
        ...
        tracker_names.append(
            {"name": trk.name, "param": trk.parameter_name, "run_id": trk.run_id, "disp_name": trk.display_name}
        )
    ...
    plot_draw_save(
        auc_curve, threshold_set_overlap, scores, tracker_names, plot_draw_styles, result_plot_path, success_plot_opts
    )
    plt.show()


def print_per_sequence_results(trackers, dataset, report_name, merge_results=False, filter_criteria=None, **kwargs):
    ...
    sequence_names = [
        s + " (ID={})".format(i) for i, (s, v) in enumerate(zip(sequence_names, valid_sequence.tolist())) if v
    ]
...
def plot_attributes_radar(
    trackers, datasets, report_name, merge_results=False, plot_opts=None, verbose=False, **kwargs
):
...
def plot_radar_draw_save(scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    avg_score_per_tracker = torch.Tensor(
        [torch.mean(torch.Tensor([scores[a][i] for a in attributes])) for i in range(len(trackers))]
    )
    ...
    for id, id_sort in enumerate(index_sort):
        ...
        if len(disp_name) > 3 and disp_name[:3] == r"\bf":
            legend_text.append(
                r"$\mathbf{{{}}}$ $\mathbf{{[{:.1f}]}}$".format(disp_name[3:], avg_score_per_tracker[id_sort])
            )
        else:
    ...
    ax.legend(
        ...
        ncol=min(len(trackers), 3),
    )

    fig.tight_layout()
    fig.savefig(
        "{}/radar_plot.pdf".format(result_plot_path), dpi=300, format="pdf", transparent=True, bbox_inches="tight"
    )
    plt.draw()
