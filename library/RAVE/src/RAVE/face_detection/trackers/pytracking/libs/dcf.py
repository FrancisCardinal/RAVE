...
def hann2d_clipped(sz: torch.Tensor, effective_sz: torch.Tensor, centered=True) -> torch.Tensor:
    ...
    effective_window = hann1d(effective_sz[0].item(), True).reshape(1, 1, -1, 1) * hann1d(
        effective_sz[1].item(), True
    ).reshape(1, 1, 1, -1)
...
def label_function(sz: torch.Tensor, sigma: torch.Tensor):
    return gauss_fourier(sz[0].item(), sigma[0].item()).reshape(1, 1, -1, 1) * gauss_fourier(
        sz[1].item(), sigma[1].item(), True
    ).reshape(1, 1, 1, -1)


def label_function_spatial(
    sz: torch.Tensor, sigma: torch.Tensor, center: torch.Tensor = torch.zeros(2), end_pad: torch.Tensor = torch.zeros(2)
):
    """The origin is in the middle of the image."""
    return gauss_spatial(sz[0].item(), sigma[0].item(), center[0], end_pad[0].item()).reshape(
        1, 1, -1, 1
...
def get_interp_fourier(
    sz: torch.Tensor, method="ideal", bicubic_param=0.5, centering=True, windowing=False, device="cpu"
):
...
def get_reg_filter(sz: torch.Tensor, target_sz: torch.Tensor, params):
    ...
    reg_window = (params.reg_window_edge - params.reg_window_min) * (
        ...
        + torch.abs(wcg / reg_scale[1]) ** params.reg_window_power
