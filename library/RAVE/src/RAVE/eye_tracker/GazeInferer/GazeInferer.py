...
class GazeInferer:
    def __init__(self, ellipse_dnn, dataloader, device, eyeball_model_path = "model_01.json", image_scaling_factor=1, pupil_radius=4, initial_eye_z=49.497, flen=3.37, sensor_size=(2.7216, 3.6288)):
    ...
    def fit(self):
        with torch.no_grad():
            for images, labels, _ in tqdm(self._dataloader, "Adding to fitting", leave=False):
                images = images.to(self._device)
                labels = labels.to(self._device)

                # Forward Pass
                predictions = labels # self._ellipse_dnn(images)
        ...
        self._eyefitter.fit_projected_eye_centre(ransac=True, max_iters=1000, min_distance=10* len(self._dataloader.dataset))
