...
class EyeTrackerDatasetBuilder(DatasetBuilder):
    ...
    def get_builders():
        ...
        """
        SOURCE_DIR = os.path.join(EyeTrackerDataset.EYE_TRACKER_DIR_PATH, 'LPW')

        TRAINING_PATH   = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH, EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, TRAINING_DIR)
        VALIDATION_PATH = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH, EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, VALIDATION_DIR)
        TEST_PATH       = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH, EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, TEST_DIR)

        if( os.path.isdir(TEST_PATH) ): 
            print('dataset found on disk')
            return -1 

        BUILDERS = [ EyeTrackerDatasetBuilderOfflineDataAugmentation(TRAINING_VIDEOS, TRAINING_PATH, 'training   dataset', IMAGE_DIMENSIONS[1:3], SOURCE_DIR), 
                     EyeTrackerDatasetBuilder(VALIDATION_VIDEOS, VALIDATION_PATH, 'validation dataset', IMAGE_DIMENSIONS[1:3], SOURCE_DIR), 
                     EyeTrackerDatasetBuilder(TEST_VIDEOS, TEST_PATH, 'test       dataset', IMAGE_DIMENSIONS[1:3], SOURCE_DIR ) ]
        return BUILDERS

   
    def process_image_label_pair(self, processed_frame, file_name, annotations,  INPUT_IMAGE_WIDTH, INPUT_IMAGE_HEIGHT):
        angle, center_x, center_y, ellipse_width, ellipse_height = self.parse_current_annotation(annotations)

        if(angle == -1): 
            return # The annotation files use '-1' when the pupil is not visible on a frame. 
        ...
        self.save_image_label_pair(file_name, processed_frame, self.current_ellipse.to_list())
...
class EyeTrackerDatasetBuilderOfflineDataAugmentation(EyeTrackerDatasetBuilder):
