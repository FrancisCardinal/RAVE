...
class EyeTrackerDatasetBuilder(DatasetBuilder):
    ...
    def get_builders():
        ...
        TRAINING_PATH = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH,
                                     EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, TRAINING_DIR)
        VALIDATION_PATH = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH,
                                       EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, VALIDATION_DIR)
        TEST_PATH = os.path.join(EyeTrackerDatasetBuilder.ROOT_PATH,
                                 EyeTrackerDataset.EYE_TRACKER_DIR_PATH, DATASET_DIR, TEST_DIR)
        ...
        BUILDERS = [EyeTrackerDatasetBuilderOfflineDataAugmentation(TRAINING_VIDEOS, TRAINING_PATH, 'training   dataset', IMAGE_DIMENSIONS[1:3], SOURCE_DIR),
                    EyeTrackerDatasetBuilder(
                        VALIDATION_VIDEOS, VALIDATION_PATH, 'validation dataset', IMAGE_DIMENSIONS[1:3], SOURCE_DIR),
    ...
    def process_image_label_pair(self, processed_frame, file_name, annotations,  INPUT_IMAGE_WIDTH, INPUT_IMAGE_HEIGHT):
        angle, center_x, center_y, ellipse_width, ellipse_height = self.parse_current_annotation(
            annotations)
        ...
        self.current_ellipse = NormalizedEllipse.get_normalized_ellipse_from_opencv_ellipse(
            center_x, ellipse_width, center_y, ellipse_height, angle, INPUT_IMAGE_WIDTH, INPUT_IMAGE_HEIGHT)
