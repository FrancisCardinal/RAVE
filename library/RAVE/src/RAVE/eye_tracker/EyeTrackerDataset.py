...
from RAVE.eye_tracker.EyeTrackerVideoCapture import EyeTrackerVideoCapture
...
class EyeTrackerInferenceDataset(EyeTrackerDataset):
    ...
    def __init__(self, opencv_device, is_real_time=True):
        ...
        self._video_feed = EyeTrackerVideoCapture(opencv_device, self.ACQUISITION_WIDTH, self.ACQUISITION_HEIGHT)
        self._length = 1
        if not is_real_time:
            self._length = (
                int(self._video_feed.get(30)) - 1
            )
