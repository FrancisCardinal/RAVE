...
class Trainer():
    ...
    def train_with_validation(self, NB_EPOCHS):
        ...
        print(figure_title)
        plt.savefig(os.path.join(self.ROOT_DIR_PATH,
                                 Trainer.TRAINING_SESSIONS_DIR, figure_title + '.png'), dpi=200)
        plt.close(None)
    ...
    def update_plot(self):
        ...
        """
        plt.clf()
        plt.plot(range(len(self.training_losses)),
                 self.training_losses, label='training loss')
        plt.plot(range(len(self.validation_losses)),
                 self.validation_losses, label='validation loss')
    ...
    def load_best_model(model, MODEL_DIR_PATH):
        ...
        checkpoint = torch.load(os.path.join(
            MODEL_DIR_PATH, Trainer.MODEL_INFO_FILE_NAME))
