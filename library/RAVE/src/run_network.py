...
def main(TRAIN, NB_EPOCHS, CONTINUE_TRAINING, DISPLAY_VALIDATION, TEST, GPU, DATASET_PATH):
    ...
    training_sub_dataset, validation_sub_dataset = torch.utils.data.random_split(
        dataset, [lenght_dataset - validation_size, validation_size]
    )
    ...
    if TRAIN:
        optimizer = torch.optim.Adam(audioModel.parameters(), lr=2e-03)
    ...
    AudioTrainer.load_best_model(audioModel, directory)
    ...
    with torch.no_grad():
        for audios, labels, _ in data_loader:
            ...
            for audio, prediction, label in zip(audios, predictions, labels):
                ...
                y, x = np.mgrid[slice(0, 513, 1), slice(0, dataset.duration, dataset.duration / dataset.nb_chunks)]
                y2, x2 = np.mgrid[slice(0, 1026, 1), slice(0, dataset.duration, dataset.duration / dataset.nb_chunks)]
...
def test(model, data_loader, DEVICE, dataset):
    ...
    audio_signal, _, _, _, _, _, _ = dataset.load_item_from_disk(item_path)
